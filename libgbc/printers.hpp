...
namespace gbc
{
  ...
  static const char* cstr_cond_en(const uint8_t bf, const uint8_t flags) {
    ...
    switch (f) {
      case 0: return (flags & 0x80) == 0 ? "YES" : "NO";
      case 1: return (flags & 0x80) ? "YES" : "NO";
      case 2: return (flags & 0x10) == 0 ? "YES" : "NO";
      case 3: return (flags & 0x10) ? "YES" : "NO";
    }
                               ...
                               const uint8_t opcode, const uint8_t flags)
  {
    snprintf(buffer, len, "%s: %s",
             cstr_cond(opcode), cstr_cond_en(opcode, flags));
  }
